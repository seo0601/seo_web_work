...
public class TaskDao {
	...
	public int getCount() {
		...
		try {
			...
			//실행할 sql 문의 뼈대 구성하기
			String sql = "SELECT MAX(ROWNUM) AS num FROM task";
   ...
   //글 하나의 정보를 리턴해주는 메소드
   public TaskDto getData(int num) {
      TaskDto dto=null;
      Connection conn = null;
      PreparedStatement pstmt = null;
      ResultSet rs = null;
	 ...
	 //글의 목록을 리턴하는 메소드
	 public List<TaskDto> getList(TaskDto dto){
	...
	//새 글을 저장하는 메소드
	public boolean insert(TaskDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int rowCount = 0;
		...
		try {
			...
			String sql = "INSERT INTO task"
					...
					+ " VALUES(task_seq.NEXTVAL, ?, ?, ?, 0, SYSDATE)";
			pstmt = conn.prepareStatement(sql);
			//?에 바인딩 할 것이 있으면 해주고
