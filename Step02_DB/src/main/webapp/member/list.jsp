...
<style>
	...
	.icon-link svg{
	  	/*transform을 적용할때 0.4초 동안 변화되도록 적용해라*/
...
<body>
	<%--/include/navbar.jsp 포함시키기
		...
		- 파라미터를 적절히 전달해서 포함된 jsp 페이지 내에서 해당 파라미터를 추출해서 어떤 로직을 수행할 수 있다.
	 	- 아래 같은 경우는 thisPage라는 파라미터 명으로 member라는 문자열을 전달하는 것이다.
	 	  그러면 마치 navbar.jsp?thisPage=member를 요청한 것과 비슷한 효과가 되어서
	 	  navbar.jsp 페이지 내부에서 
	 	  
	 	  String thispage = request.getPrameter("thisPage");
	...
	<div class="container">
		<a href="${pageContext.request.contextPath}/member/insertform.jsp" class="icon-link">
			...
			<sapn class="visually-hidden">회원 추가</sapn>
		</a>
		<h1><small>회원 목록입니다.</small></h1>
				...
				<tr>
					...
					<td>
						<a href="updateform.jsp?num=<%=tmp.getNum()%>">
							...
							<sapn class="visually-hidden">회원 수정</sapn>
						</a>
					</td>
					<td>
						...
						<sapn class="visually-hidden">회원 삭제</sapn>
					</td>
					<td>
